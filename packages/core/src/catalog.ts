    ...
    const result: Interface = {
      ...
      model: {
        ...
        default: Effect.fn("CatalogV2.model.default")(function* () {
          const deepblack = yield* result.model.get(
            ProviderV2.ID.make("deepblack"),
            ModelV2.ID.make("deepseek-reasoner"),
          )
          if (deepblack?.enabled) return deepblack
        ...
        small: Effect.fn("CatalogV2.model.small")(function* (providerID) {
          const record = state.get().providers.get(providerID) ?? state.get().providers.get(ProviderV2.ID.make("deepblack"))
          if (!record) return
          const provider = record.provider

          const chatModel = record.models.get(ModelV2.ID.make("deepseek-chat"))
          if (chatModel?.enabled && chatModel.status === "active") return projectModel(chatModel, provider)
