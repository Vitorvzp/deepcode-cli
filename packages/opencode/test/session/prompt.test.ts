...
// /reasoning and /searching toggles

noLLMServer.instance(
  "threads thinkingEnabled/searchEnabled onto the user message's model",
  () =>
    Effect.gen(function* () {
      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
      ...
      const neither = yield* prompt.prompt({
        sessionID: session.id,
        agent: "build",
        noReply: true,
        parts: [{ type: "text", text: "hello" }],
      })
      if (neither.info.role !== "user") throw new Error("expected user message")
      expect(neither.info.model.thinkingEnabled).toBeUndefined()
      expect(neither.info.model.searchEnabled).toBeUndefined()

      const both = yield* prompt.prompt({
        sessionID: session.id,
        agent: "build",
        noReply: true,
        thinkingEnabled: true,
        searchEnabled: true,
        parts: [{ type: "text", text: "hello with both on" }],
      })
      if (both.info.role !== "user") throw new Error("expected user message")
      expect(both.info.model.thinkingEnabled).toBe(true)
      expect(both.info.model.searchEnabled).toBe(true)

      const thinkingOnly = yield* prompt.prompt({
        sessionID: session.id,
        agent: "build",
        noReply: true,
        thinkingEnabled: true,
        searchEnabled: false,
        parts: [{ type: "text", text: "hello with only thinking on" }],
      })
      if (thinkingOnly.info.role !== "user") throw new Error("expected user message")
      expect(thinkingOnly.info.model.thinkingEnabled).toBe(true)
      expect(thinkingOnly.info.model.searchEnabled).toBe(false)
