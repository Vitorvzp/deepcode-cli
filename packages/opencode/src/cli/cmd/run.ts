  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("reasoning", {
        type: "boolean",
        describe:
          "enable provider-side extended thinking for this run (e.g. DeepBlack thinking_enabled) — same effect as the /reasoning command in interactive mode, but set once for a headless run",
      })
      .option("searching", {
        type: "boolean",
        describe:
          "enable provider-side web search for this run (e.g. DeepBlack search_enabled) — same effect as the /searching command in interactive mode, but set once for a headless run",
      })
      .option("mini", {
        type: "boolean",
        hidden: true,
      ...
      async function execute(sdk: OpencodeClient) {
        ...
        if (!interactive) {
          ...
          const result = await client.session.prompt({
            ...
            model,
            variant: args.variant,
            thinkingEnabled: args.reasoning,
            searchEnabled: args.searching,
