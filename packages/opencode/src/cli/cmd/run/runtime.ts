  ...
  const runQueue = async () => {
    ...
    await mod.runPromptQueue({
      ...
      onNewSession: createSession
        ? async () => {
            try {
              ...
              state.history = []
              state.localRows = []
              // /reasoning and /searching are scoped to the session they were
              // toggled in — a fresh session starts with both off, same as
              // activeVariant resets on model switch.
      ...
      onToggleThinking: () => {
        ...
        return { status: `reasoning ${state.thinkingEnabled ? "ativado" : "desativado"}` }
      },
      onToggleSearching: () => {
        state.searchEnabled = !state.searchEnabled
        return { status: `searching ${state.searchEnabled ? "ativado" : "desativado"}` }
      },
