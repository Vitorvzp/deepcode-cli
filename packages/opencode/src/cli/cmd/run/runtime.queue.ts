...
import { isExitCommand, isNewCommand, isReasoningCommand, isSearchingCommand } from "./prompt.shared"
import type { FooterApi, FooterEvent, FooterQueuedPrompt, RunPrompt } from "./types"

type ToggleResult = { status?: string } | void
...
export type QueueInput = {
  ...
  onToggleThinking?: () => ToggleResult
  onToggleSearching?: () => ToggleResult
...
export async function runPromptQueue(input: QueueInput): Promise<void> {
  ...
  const submit = (prompt: RunPrompt) => {
    ...
    // /reasoning and /searching are instant, local toggles — no LLM turn is
    // sent. The flipped flag only takes effect starting with the *next*
    // ordinary prompt (threaded through SessionTurnInput -> PromptInput ->
    // providerOptions.openai on that turn).
    if (prompt.mode !== "shell" && isReasoningCommand(prompt.text)) {
      const status = input.onToggleThinking?.()?.status ?? "reasoning toggled"
      emit({ type: "stream.patch", patch: { status } }, { status })
      return
    }
    if (prompt.mode !== "shell" && isSearchingCommand(prompt.text)) {
      const status = input.onToggleSearching?.()?.status ?? "searching toggled"
      emit({ type: "stream.patch", patch: { status } }, { status })
      return
    }
