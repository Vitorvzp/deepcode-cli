...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const getSmallModel = Effect.fn("Provider.getSmallModel")(function* (providerID: ProviderV2.ID) {
      ...
      const s = yield* InstanceState.get(state)
      const provider = s.providers[providerID] ?? s.providers[ProviderV2.ID.make("deepblack")]
      if (!provider) return undefined

      if (provider.models[ModelV2.ID.make("deepseek-chat")]) return provider.models[ModelV2.ID.make("deepseek-chat")]
    ...
    const defaultModel = Effect.fn("Provider.defaultModel")(function* () {
      ...
      const s = yield* InstanceState.get(state)
      if (s.providers[ProviderV2.ID.make("deepblack")]?.models[ModelV2.ID.make("deepseek-reasoner")]) {
        return {
          providerID: ProviderV2.ID.make("deepblack"),
