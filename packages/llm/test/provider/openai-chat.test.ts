...
describe("OpenAI Chat route", () => {
  ...
  it.effect("maps DeepBlack thinking/search toggles to Chat body fields", () =>
    Effect.gen(function* () {
      const prepared = yield* LLMClient.prepare<OpenAIChat.OpenAIChatBody>(
        LLM.request({
          model,
          prompt: "search the web and think it through",
          providerOptions: { openai: { thinkingEnabled: true, searchEnabled: false } },
        }),
      )

      expect(prepared.body.thinking_enabled).toBe(true)
      expect(prepared.body.search_enabled).toBe(false)
    }),
  )

  it.effect("omits thinking/search fields entirely when the toggles were never set", () =>
    Effect.gen(function* () {
      const prepared = yield* LLMClient.prepare<OpenAIChat.OpenAIChatBody>(request)

      expect(prepared.body.thinking_enabled).toBeUndefined()
      expect(prepared.body.search_enabled).toBeUndefined()
    }),
  )
