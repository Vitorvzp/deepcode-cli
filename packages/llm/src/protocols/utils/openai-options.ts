...
// Backing an OpenAI-compatible provider's own extended-thinking / web-search
// toggles (e.g. the DeepBlack bridge's `thinking_enabled` / `search_enabled`
// fields), set per-turn via the /reasoning and /searching commands. Not part
// of the official OpenAI Chat Completions API, so unrecognized providers
// simply ignore these fields on the wire.
export const thinkingEnabled = (request: LLMRequest): boolean | undefined => {
  const value = options(request)?.thinkingEnabled
  return typeof value === "boolean" ? value : undefined
}

export const searchEnabled = (request: LLMRequest): boolean | undefined => {
  const value = options(request)?.searchEnabled
  return typeof value === "boolean" ? value : undefined
}
