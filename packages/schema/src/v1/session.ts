...
export const User = Schema.Struct({
  ...
  model: Schema.Struct({
    ...
    // Session-scoped toggles set via /reasoning and /searching, threaded
    // straight through to providerOptions.openai.{thinkingEnabled,
    // searchEnabled} on the next turn. Currently only meaningful for
    // OpenAI-compatible providers that understand these fields (e.g. the
    // DeepBlack bridge); other providers simply ignore them.
    thinkingEnabled: Schema.optional(Schema.Boolean),
    searchEnabled: Schema.optional(Schema.Boolean),
  }),
