...
export function Autocomplete(props: {
  ...
  const options = createMemo((prev: AutocompleteOption[] | undefined) => {
    ...
    // files() only does real work in "@" mode (its fetcher early-returns []
    // for "/" -- see the createResource above). But its source signal
    // includes search(), so it re-enters "loading" on every keystroke
    // regardless of mode. Scoping this stale-cache return to "@" stops it
    // from masking freshly-computed "/" results behind a frozen snapshot
    // of whatever the command list looked like before the user typed
    // anything (root cause of /reasoning, /searching, and in fact every
    // slash command never visibly filtering by query -- confirmed upstream
    // bug via tests/fixtures, not fork-local).
